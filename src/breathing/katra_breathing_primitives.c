...
int remember(const char* ci_id, const char* thought, why_remember_t why) {
    if (!ci_id || !thought) {
        katra_report_error(E_INPUT_NULL, "remember", "ci_id or thought is NULL");
        return E_INPUT_NULL;
    }

    LOG_DEBUG("Remembering for %s (%s): %s", ci_id, why_to_string(why), thought);
...
int remember_with_note(const char* ci_id, const char* thought, why_remember_t why, const char* why_note) {
    if (!ci_id || !thought || !why_note) {
        katra_report_error(E_INPUT_NULL, "remember_with_note", "NULL parameter");
        return E_INPUT_NULL;
    }

    LOG_DEBUG("Remembering for %s (%s) with note: %s", ci_id, why_to_string(why), thought);
...
int reflect(const char* ci_id, const char* insight) {
    if (!ci_id || !insight) {
        katra_report_error(E_INPUT_NULL, "reflect", "ci_id or insight is NULL");
        return E_INPUT_NULL;
    }
...
int learn(const char* ci_id, const char* knowledge) {
    if (!ci_id || !knowledge) {
        katra_report_error(E_INPUT_NULL, "learn", "ci_id or knowledge is NULL");
        return E_INPUT_NULL;
    }
...
int decide(const char* ci_id, const char* decision, const char* reasoning) {
    if (!ci_id || !decision || !reasoning) {
        katra_report_error(E_INPUT_NULL, "decide", "NULL parameter");
        return E_INPUT_NULL;
    }
...
int notice_pattern(const char* ci_id, const char* pattern) {
    if (!ci_id || !pattern) {
        katra_report_error(E_INPUT_NULL, "notice_pattern", "NULL parameter");
        return E_INPUT_NULL;
    }
...
int remember_forever(const char* ci_id, const char* thought) {
    if (!ci_id || !thought) {
        katra_report_error(E_INPUT_NULL, "remember_forever", "NULL parameter");
        return E_INPUT_NULL;
    }
