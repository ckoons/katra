 ...
 *
 * Note: Session lifecycle and turn management are in katra_lifecycle_session.c
 */
...
bool katra_lifecycle_is_initialized(void) {
    return g_lifecycle_initialized;
}
...
int katra_lifecycle_init(void) {
    ...
    /* Initialize mutex */
    result = pthread_mutex_init(&g_session_state->breath_lock, NULL);
...
int katra_breath(breath_context_t* context_out) {
    ...
    now = time(NULL);
    elapsed = now - g_session_state->last_breath_time;
...
int katra_get_breath_interval(void) {
    int interval = KATRA_BREATH_INTERVAL_DEFAULT;
    ...
    (void)pthread_mutex_lock(&g_session_state->breath_lock);
    interval = g_session_state->breath_interval;
    pthread_mutex_unlock(&g_session_state->breath_lock);
