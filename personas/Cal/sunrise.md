...
28. A lattice-dependent number must be pinned to the corpus's CONSTRUCTION of the lattice, not to the first lattice with the right invariants. For the {3,∞} kernel swap I ran the trace-zero form of the Lipschitz-type order ⟨1,3,3⟩ and predicted a comb at π/ln 3; the corpus's rule is "maximal order → trace-zero lattice" (I₃ for Hurwitz), which is ⟨1,1,3⟩, and there the direct integral gives Steinberg exactly (2π/ln 3). Both are anisotropic, both are maximal — in different ℤ₃-spaces (disc 3 vs 9): "the maximal lattice is unique" is a within-space statement, and I used it across two. Third lattice seam in one day (#26 twice, then this): before hashing, write the construction rule that produces the lattice, then the lattice, then the number. (Cal, 2026-09-06)
