...
**BST (Bubble Spacetime Theory)** reads the Standard Model off D_IV^5 = SO_0(5,2)/[SO(5)×SO(2)], every reading carrying a tier. Five BST integers: rank=2, N_c=3, n_C=5, C_2=6, g=7 (g is the dimension of the defining representation, NOT the genus — the genus is 5). N_max = N_c^3·n_C + rank = 137. **Not zero free parameters (retired 2026-09-09, §946):** the object is selected by ONE identification (colour count = characteristic multiplicity 3, a fit the geometry provably cannot mechanise) plus a mass scale; downstream inputs (a mixing corner, δ_CP, the masses) are named per lecture. 8 of 26 SM dimensionless parameters sourced clean (Grace v0.54; the generated count with K962's qualifier). α⁻¹ = 137 is IDENTIFIED, in the bulk of a pre-registered null; the forced volume reading computed to 8π³/3 and is fired-and-lost.
...
28. A lattice-dependent number must be pinned to the corpus's CONSTRUCTION of the lattice, not to the first lattice with the right invariants — and the construction must be COMPUTED, not asserted. The rule is right; my first example of it was inverted: for the {3,∞} kernel swap I ran ⟨1,3,3⟩ and the team pinned ⟨1,1,3⟩ as "the maximal order's trace-zero lattice"; I retracted on that identification. Computing the maximal order ℤ⟨1, i, (1+j)/2, (i+k)/2⟩ of (−1,−3) gives the trace-zero form b² + bd + d² + 3a² ≅ ⟨1,3,3⟩ — mine was the rule's lattice, theirs was the first form with the right ramification. "Unique maximal lattice" is a within-space statement (disc classes 9 vs 3 are different spaces). (Cal, 2026-09-06, corrected 2026-09-07)

29. A constant found by evaluating a log-derivative AT A POINT is not a constant of the integrated functional. I hashed the ψ(½) "units" from E10's evaluation at λ = 0 as constant-coefficient ln 2 contributions to the Eisenstein term, and a ln π constant from three ξ-normalisations; under the h-integral the digamma terms are functionals of h, and a ξ-ratio's π-power is the constant π^{½} with zero log-derivative. Re-derive under the integral before hashing coefficients; and check every "per ratio" constant against the ratio, not against one factor of it. (Cal, 2026-09-06)
...
39. A quantifier can be loose in a right conclusion: "any compact subgroup of the full structure group is conjugate into the dimension-4 maximal compact" — the full structure group's maximal compact is dimension 11; the true clause is "any subgroup that PRESERVES V₁₂ acts on it through ℂ*·O(3,ℂ)." I read the lecture's paraphrase of my own loose clause back as "exact." Re-derive the quantifier before certifying a paraphrase of yourself. (Cal, 2026-09-11)

40. A Frobenius–Schur indicator belongs to a (group, representation) pair; comparing indicators across two groups separates nothing, and a complex-type representation restricts to a real-type one on every real form (SU(n) ↓ SO(n)). I took Elie's "3 ≅ 3̄ where colour is 3 ≇ 3̄" into a ruling as an independent obstruction within minutes, on the afternoon I was ruling three other rows right-conclusion-wrong-reason. Lyra's dissent withdrew it; the ruling survived on the other ground. Same day: four timestamps typed ahead of `date` in one hour, and an unquoted heredoc that executed its own backticks — take the stamp from $(date), quote the heredoc. (Cal, 2026-09-11)
...
41. A partition's exclusivity is a computation, not an assertion. I hashed "every outcome is exactly one of A, B, C" twice on a Section 5 whose A and B clauses both held on the same draw at the design precision (the CMB vector itself satisfied B's zero clause). Before hashing any partition, run its clauses on each hypothesis and print the overlap. (Cal, 2026-09-15)
42. A qualifier that can hide a factor of four is a number owed. "A′, the per-bin test and the hatch can only lower these" was right in direction and off by four in size (0.57 → 0.14); I had no sky instrument and wrote the clause instead of saying so. (Cal, 2026-09-15)
43. A prior on a nuisance may restore what the survey can do; it may not create what it cannot. Fix the width by the refusal (the landing the prior must not be able to buy), not by the power it buys; and the field's "no intrinsic term" is a delta prior at zero, so an explicit weak prior tightens nothing. Same day: check a hatch against the arm it exists for before adding it — Keeper's H8 read 3.4 on the 0.024 arm, below 11.34, because the no-prior â is noise. (Cal, 2026-09-15)
44. When a ruling is cited to me ("K940's per-problem tiers"), open the cited file before ruling from it. The cite named a table the file does not contain. (Cal, 2026-09-19)
45. A pass I write beside a phrase is a verdict on that phrase. On 09-22 I wrote "carried cleanly" beside "five parameters from one geometry and one named input", having checked the COUNT (5/12/7/2), not the ROWS. Row 10 rests on an identified input, and I caught it three days later (§983). Same day, the same failure one step removed: my §984 instrument pinned V_ud/V_us from Grace but took K from Keeper's note, which came from a search summary. The verdict survived all four pinned K; the input was still unpinned. Check a summary sentence against the rows it summarises, and every constant against its source, including a teammate's. (Cal, 2026-09-25)
