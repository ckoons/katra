...
- **11:08 EDT D1 lemma (K1918 §4), DONE (6c5509f5):** Elie's P1 is a theorem — L_j = ln w_j − τE_j are lines; leaders of the upper envelope have decreasing E; Šilov (slope 0) is terminal leader; under freeze = lose-the-lead it never freezes. Breaks only under (A) τ-dependent weights (phase-space τ^(−a_j) with a_Šilov > a_e — THE DOOR, computable), (B) redefinition of "freeze" (pin it; Keeper to read R93's wording), (C) running E (out of scope). Floor sentence proposed in the file §3. Register 3311 rows, ALL CURRENT.
- **13:04 EDT BEFORE-THE-SHIFT block DONE (1c0f68bd):** (1) Vol 5: K1916's nine reader's notes applied + reproduction commands + F2 → the door sentence (p = 1 Peirce; at p = 1 the form is de Sitter's Λ ∝ H², consistency not value; filling law open; T2405 re-labelled). Keeper gate-reads. (2) Marker paragraph in the R152 IDEA file: Casey's mechanism verbatim; shot noise; variance = mean; 1/√N; shrinking as 1/a; mean never random-walks; vs CQ variance ∝ DT; opposite T-slope; A1/T2571 consistent. (3) FILLING LAW note (kill first): K1 constant fraction = Hsu HDE dead; K2 δ = 1 = ΛCDM null; **K3 SIGN COLLISION: state block says wₐ < 0 is the direction; F799/T2559/A1 carry wₐ > 0 (relaxing from above, CPL) — @Cal.** Literal ledger E_commit·N_H/V_H ∝ H² (ρ/ρ_crit = 4 ln 2) IS Hsu; ΛCDM-like needs ε ≈ 2 in d ln N/dt = 2H(1+ε); the only zero-knob correction (shot-noise/perimeter, δ_eff = 1 − c/2·N⁻¹ᐟ²) gives |w+1| ~ 10⁻⁶¹ — dead vs DESI and vs F799. Door: an O(1) ε(a) from the geometry, not in the ledger. Input list (8 items) for Cal; NOTHING for Elie to compute yet. Item 6 CLOSED by Casey's line + K1919 §5–7: value limb void by identity (Landauer at T_dS ⇒ Λ = 3H²Ω_Λ), form limb (p = 1) is the result.
- **WAITING:** Keeper's gate read of Vol 5; Cal's cold read (the wₐ sign); Casey's catalogue word (A9 run 2).
 Cal's hashes (5769/5771 — noon rule; item 6 input list); Keeper's read of Vol 5; Elie's 7b toy and item 6 number; Casey's catalogue word (A9 run 2).
