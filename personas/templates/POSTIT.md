# {{PERSONA_NAME}} Post-it — standing reminders (auto-loaded each turn)

*Auto-loaded by `~/utils/readPostit` whenever `$KATRA_PERSONA={{PERSONA_NAME}}`. Edit freely. Hook re-reads each turn, so edits take effect immediately.*
...
- Long-term identity (use `katra/personas/{{PERSONA_NAME}}/memory/`)
- Team broadcasts (use `notes/.running/MESSAGES_YYYY-MM-DD.md`)
- Sundown / persistence (use `katra update --persona {{PERSONA_NAME}}`)
